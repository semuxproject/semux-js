...
export const SemuxApiFp = function(configuration?: Configuration) {
    return {
        ...
        createAccount(name?: string, options?: any): (fetch?: FetchAPI, basePath?: string) => Promise<CreateAccountResponse> {
            const localVarFetchArgs = SemuxApiFetchParamCreator(configuration).createAccount(name, options);
            return (fetch: FetchAPI = portableFetch, basePath: string = BASE_PATH) => {
                return fetch(basePath + localVarFetchArgs.url, localVarFetchArgs.options).then((response) => {
                    if (response.status >= 200 && response.status < 300) {
                        return response.json();
...
export const SemuxApiFactory = function (configuration?: Configuration, fetch?: FetchAPI, basePath?: string) {
    return {
        ...
        createAccount(name?: string, options?: any) {
            return SemuxApiFp(configuration).createAccount(name, options)(fetch, basePath);
        },
        /**
         * Returns an account.
         * @summary Get account
