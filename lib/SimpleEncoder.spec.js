...
describe("SimpleEncoder", () => {
  ...
  it("encodes a byte array", () => {
    const TEST_BYTES = [-103, -101, -48, -94, 122, -22, 126, 109, -71, 64, -70, 18, 109, -116, -49, 70];
    const TEST_BYTES_HEX = "10999bd0a27aea7e6db940ba126d8ccf46";

    const encoder = new SimpleEncoder();
    encoder.writeBytes(new Uint8Array(TEST_BYTES));
    Assert.deepEqual(encoder.toBytes(), Buffer.from(TEST_BYTES_HEX, "hex"));
  });
